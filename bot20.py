...
def create_temp_text_file(filename):
    """Создание временного файла с принудительной кодировкой UTF-8 BOM"""
    temp_dir = os.path.join(os.path.dirname(__file__), "temp")
    os.makedirs(temp_dir, exist_ok=True)
    temp_path = os.path.join(temp_dir, filename)

    try:
        # Создаем файл с BOM-маркером UTF-8
        with open(temp_path, "w", encoding="utf-8-sig") as f:
            f.write("")

        # Явно открываем в Блокноте с указанием кодировки (для Windows)
        if os.name == 'nt':
            os.system(f'notepad.exe "{temp_path}"')
        else:
            os.system(f'xdg-open "{temp_path}"')

        input("Нажмите Enter после сохранения файла...")
...
def create_config():
    ...
    # 11. Ввод названия конфига
    clear_console()
    while True:
        print("Введите название конфига (только латинские буквы, цифры и подчёркивания)")
        config_name = input("Имя файла (без 'config_' и расширения): ").strip()
        ...
        if not re.match(r'^[a-zA-Z0-9_-]+$', config_name):
            print("Недопустимые символы! Используйте только латинские буквы, цифры, '-', '_'")
            continue
            
        full_name = f"config_{config_name}.json"
        config_path = os.path.join(script_dir, "configs", full_name)
...
def auto_posting_manager():
    ...
    board_scanners = {}
    running_configs = {}  # Должен быть пустым при инициализации
    script_dir = os.path.dirname(os.path.abspath(__file__))
    configs_dir = os.path.join(script_dir, 'configs')

    def load_states():  # Эта функция ДОЛЖНА быть здесь
        try:
    ...
    running_configs = load_states()  # ПОСЛЕ определения функции


def auto_posting_manager():
    STATE_FILE = "config_states.json"
    board_scanners = {}
    running_configs = {}  # Начинаем с пустого словаря
    script_dir = os.path.dirname(os.path.abspath(__file__))
    configs_dir = os.path.join(script_dir, 'configs')
    ...
    def run_configs():
        print("[DEBUG] Запуск процесса постинга")
        while running_configs:
            for filename, settings in list(running_configs.items()):
                ...
                try:
                    if settings.get('mode') == '2':
                        min_int = settings.get('min_interval', 300)
                        max_int = settings.get('max_interval', 360)
                        print(f"[DEBUG] Обработка {filename} в manual режиме")
                        success = process_config(config_path)
                        if success:
                            process_board_page(config_path)
                            delay = random.randint(min_int, max_int)
                            print(f"[DEBUG] Ожидание {delay} секунд")
                            time.sleep(delay)
                except Exception as e:
                    print(f"[ERROR] Ошибка в {filename}: {str(e)}")
            time.sleep(1)
    ...
    # ПОСЛЕ определения всех функций обновляем running_configs
    running_configs.update(load_states())
    print(f"[DEBUG] Загружены конфиги: {running_configs}")

    posting_thread = threading.Thread(target=run_configs)
    posting_thread.daemon = True
    posting_thread.start()
    print("[DEBUG] Поток постинга запущен")
